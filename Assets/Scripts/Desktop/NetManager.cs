...
public class NetManager
{
    ...
    public void Connect(string uriStr)
    {
        ...
        catch (Exception ex)
        {
            ...
            Debug.LogError("WebSocket状态：" + m_clientWebSocket.State);
            //关闭连接
    ...
    private string Receive(ClientWebSocket socket)
    {
        try
        {
            ...
            {
                byte[] arrry = new byte[3072];  //注意长度，如果服务器发送消息过长，这也需要跟着调整
                ...
                Debug.Log("socket当前状态：" + socket.State);
    ...
    public void CloseClientWebSocket()
    {
        ...
        if (m_clientWebSocket != null && m_clientWebSocket.State == WebSocketState.Open)
        {
            ...
            Debug.Log("socket当前状态：" + m_clientWebSocket.State);
            task.Wait();
            Debug.Log("socket当前状态：" + m_clientWebSocket.State);
            Debug.Log("连接已断开。");
        }
